...
- (BOOL)_createCommentLayer:(NSDictionary*)comment setPosition:(BOOL)setPosition pause:(BOOL)pause;
...
@implementation NicoCommentManager
- (id)init {
    self = [super init];
    if (self) {
...
+ (id)nicoCommentManagerWithComments:(NSArray*)comments delegate:(id<NicoCommentManagerDelegate>)delegate {
    return [[NicoCommentManager alloc] initWithComments:comments delegate:delegate];
}

- (void)dealloc {
    LOG(@"NicoCommentManager dealloc");
}
...
- (void)_commentTimerFired:(NSTimer*)timer {
    ...
    CGFloat currentTime = floor(_moviePosition * 100.f);
...
- (BOOL)_createCommentLayer:(NSDictionary*)comment setPosition:(BOOL)setPosition pause:(BOOL)pause {
    ...
    if (commentPosition == COMMENT_POSITION_NORMAL) {
      ...
      duration = COMMENT_TOP_OR_BOTTOM_DURATION;
    ...
    if (commentPosition == COMMENT_POSITION_NORMAL) {
      CGFloat animateDuration = COMMENT_DURATION;

      if (setPosition) {
        CGFloat currentTime = _moviePosition * 100.f;
        CGFloat diff = (currentTime - commentTextLayer.vpos) / 100.f;
        CGFloat perWidth = (_screenSize.width + commentTextLayer.frame.size.width) / COMMENT_DURATION;

        commentOffset.x = (_screenSize.width) - (perWidth * diff);
        animateDuration = COMMENT_DURATION - diff;
      }
    ...
    @synchronized(_showCommentItems) {
      for (NSDictionary *item in _showCommentItems) {
        ...
        if (commentTextLayer.commentPosition == COMMENT_POSITION_NORMAL) {
          ...
          if (currentTime > (CGFloat)(commentTextLayer.vpos + [item[@"duration"] doubleValue] * 100.0)) {
            [commentTextLayer removeFromSuperlayer];
            [deleteItems addObject:item];
          }
        }
...
- (void)seekCommentLayer:(BOOL)pause {
    ...
    CGFloat currentTime = floor(_moviePosition * 100.f);
    CGFloat normalCommentTime = currentTime - (COMMENT_DURATION * 100.f);
    CGFloat topBottomCommentTime = currentTime - (COMMENT_TOP_OR_BOTTOM_DURATION * 100.f);
    NSInteger max = [_comments count];
    NSInteger i;
    ...
    for (i = 0; i < max; i++) {
      ...
      if (normalCommentTime < [comment[@"vpos"] floatValue] && [comment[@"position"] intValue] == COMMENT_POSITION_NORMAL) {
        [self _createCommentLayer:comment setPosition:YES pause:pause];
      }
