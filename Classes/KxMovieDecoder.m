...
- (void)decodeSeek:(CGFloat)position;
@end
...
//        avformat_seek_file(_formatCtx, _videoStream, ts, ts, ts, AVSEEK_FLAG_FRAME);
        avformat_seek_file(_formatCtx, _videoStream, INT64_MIN, ts, ts, AVSEEK_FLAG_BACKWARD);
            ...
            error: (NSError **) perror
{
    ...
    _videoCacheManager = [[VideoCacheManager alloc] init];
...
- (kxMovieError) openInput: (NSString *) path
{
    ...
    int ret;
    if (_isNetwork) {
      if ([_videoCacheManager open:path]) {
        formatCtx->pb = _videoCacheManager.ioContext;
        ret = avformat_open_input(&formatCtx, "dummy", NULL, NULL);
      }
      else {
        return kxMovieErrorOpenFile;
      }
    ...
    if (ret < 0) {
//    if (avformat_open_input(&formatCtx, "dummy", NULL, &opts) < 0) {
        if (formatCtx) {
            [_videoCacheManager close];
...
-(void) closeFile
{
    ...
    if (_videoCacheManager) {
      [_videoCacheManager close];
      _videoCacheManager = nil;
    }
...
- (NSArray *) decodeFrames: (CGFloat) minDuration
{
    ...
    while (!finished) {
        ...
        if (packet.stream_index ==_videoStream && !self.backgroundMode) {
    ...
    while (!finished) {
        ...
        if (packet.stream_index ==_videoStream && !self.backgroundMode) {
            ...
            while (pktSize > 0) {
                ...
                if (gotframe) {
                  CGFloat videoPosition = av_frame_get_best_effort_timestamp(_videoFrame) * _videoTimeBase;

                  if (position <= videoPosition) {
                    _position = videoPosition;
                    finished = YES;
                    break;
                  }
                }
